...
module.exports.addInstanceToJson = function(id, port, json_path, cb) {
  ...
  var instance = {
    ...
    script:           path.resolve(__dirname, '..', '..', 'castor-cli', 'bin',"castor"),
    node_args:        [],
    exec_mode:        "fork_mode",
  ...
  module.exports.getInstanceFromJson(id, pm2_json_path,
    function afterGetInstanceFromJson (err, instance) {
      ...
      pm2.connect(function afterConnect(err) {
        ...
        /**
         * PM2 startJson function (a JSON object is given)
         * @param  {String}   cmd     JSON object
         *                            .env
         *                            .script
         *                            .port
         *                            .cwd
         ...
         *                            .watch (Boolean)
         * @param  {String}   jsonVia "pipe" or anything else ('file')
         * @param  {Function} cb      fn(err)
         */
        // function startJson (cmd, opts, jsonVia, cb)
        pm2.startJson(
          JSON.stringify(instance),
          /*{ watch: instance.watch}*/ {},
          'pipe',
          function afterStartJson(err, data) {
            if (err) {
              pm2.disconnect(function () {
                cb(err);
              });
...
module.exports.restart = function restart (id, cb) {
  ...
  // Because pm2.restart did not keep the same port used.
  ...
  //   pm2.restart(
  //     id,
  //     function afterRestart(err) {
  //       debug('afterRestart',err);
  //       if (err) {
  //         pm2.disconnect(function () {
  //           cb(err);
  //         });
