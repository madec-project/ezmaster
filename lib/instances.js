 ...
 */
var getInstanceConfig = module.exports.getInstanceConfig = function getInstanceConfig(id) {
...
var createConfigJson =
  function createConfigJson(instance_path, id, title, app, accel, users, date, connectedUser, cb) {
  ...
  if (!app || app === '') {
    cb(new Errors.BadParameters("No app name for the instance!"));
    return;
  }
  ...
  createConfigLockFile(instance_path, function (err) {
    ...
    var dataPath = path.resolve(config.instances_path, id);
    debug('dataPath', dataPath);
    configJson.data_path = dataPath;
    ...
    if (Object.has(instancesConfigs, id)) {
      ...
      debug('users3', users);
      debug('configJson', configJson);
      configJson.connectionURI = 'mongodb://localhost:27017/' + id;
      var max_port = 35268;
      ...
      if(!app) { // For legacy instances (until castor-admin 0.5.0)
        configJson.theme = config.theme;
      }
    }
  });
};
...
/**
 * Update instance id.json
 * @param  {String}   id   Identifier of the instance
 * @param  {Object}   json New configuration of the instance
 * @param  {Function} cb   fn(err)
 */
var updateConfigJson = function updateConfigJson(id, json, cb) {
  var instance_path = path.resolve(__dirname, '..', config.instances_path, id);
  existConfigLockFile(instance_path, function updateConfig(isLocked, instanceExists) {
    if (isLocked) {
    ...
    if (!instanceExists) {
      cb(new Errors.NotFound('Instance ' + id + ' does not exist.'));
      return;
    }
    createConfigLockFile(instance_path, function afterLock (err) {
      if (err) {
        cb(new Errors.Forbidden('Unable to create lock file'));
        return;
      }
      // Core of the update
      writeConfigJson(instance_path, json, function afterWrite (err) {
        if (err) {
          cb(err);
          return;
        }
        removeConfigLockFile(instance_path, function afterUnlock (err) {
          if (err) {
            cb(err);
            return;
          }
          processes.restart(id, function afterRestart (err) {
            cb (err);
          });
        });
      });
 ...
 * @param {String}   id             identifier of the instance (alphanumeric)
 * @param {String}   title          Title of the instance (human readable)
 * @param {String}   app            Name of the app to use
...
function setInstanceConfig(id, title, app, accel, users, date, connectedUser, cb) {
  ...
  existConfigLockFile(instance_path, function createConfig(isLocked, instanceExists) {
    ...
    if (instanceExists) {
      createConfigJson(instance_path, id, title, app, accel, users, date, connectedUser, cb);
    }
    else {
...
module.exports.postRecord = function postRecord(req, res, next) {
  ...
  if (!technicalName.match(/^[a-z\-0-9]+$/)) {
    debug('technicalName', technicalName);
  ...
  setInstanceConfig(technicalName, title, app, accel, users, date, connectedUser,
 ...
 * @param  {Request}    req  request (with a params.id)
 * @param  {Response}   res  response to send
 * @param  {Function}   next Call the next Express's middleware in case of error
 */
...
/**
 * Modify the req.params.id instance JSON file
 * @param  {Request}  req  request (with params.id)
 * @param  {Response} res  response to send
 * @param  {Function} next Call the next Express's middleware in case of error
 */
module.exports.routePutRecord = function routePutRecord(req, res, next) {
  debug('routePutRecord START');
  var id   = req.params.id;
  debug('req.body', req.body);
  var json = req.body;
  debug('json', json);
  updateConfigJson(id, json, function afterUpdateConfigJson(err) {
    if (err) {
      debug('afterUpdateConfigJson', err);
      next(err);
      return;
    }
    res.send();
