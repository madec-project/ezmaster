...
var createConfigJson =
  ...
  createConfigLockFile(instance_path, function (err) {
    ...
    if (Object.has(instancesConfigs, id)) {
      ...
      writeConfigJson(instance_path, configJson, cb);
    }
    else {
...
module.exports.postRecord = function postRecord(req, res, next) {
  ...
  setInstanceConfig(technicalName, title, accel, users, date, connectedUser, function (err1) {
    debug("err", err1);
    if (err1) {
      return next(err1);
    }
